...
import { getComponentName, isWebAwesomeTag } from './component-prefix.js';
...
export async function discover(root: Document | Element | ShadowRoot) {
  ...
  const rootIsWebAwesomeComponent = isWebAwesomeTag(rootTagName);
...
function register(tagName: string): Promise<void> {
  ...
  const tagWithoutPrefix = getComponentName(tagName);
