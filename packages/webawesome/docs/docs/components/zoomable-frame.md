...
默认情况下，框架不会将主题类同步到 iframe 中。添加 `with-theme-sync` 属性以将主机页面的明暗模式和主题选择器类（如 `wa-theme-*`、`wa-brand-*` 和 `wa-palette-*`）镜像到 iframe 文档中。当 iframe 渲染需要与主机页面主题匹配的 Web Awesome 样式时，这很有用。
