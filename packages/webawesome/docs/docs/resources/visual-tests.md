...
构建和使用 @SITE_NAME@ 组件的方式多种多样，可视化测试有助于确保一致性，并防止错误的样式泄露到生产环境中。

这些测试在创建或自定义您自己的主题时会派上用场。逐个查看每个测试用例，确保您主题中的自定义样式涵盖了 @SITE_NAME@ 提供的所有特性、工具类和内置样式。
...
原生样式测试确保支持的原生元素和工具类与对应的 @SITE_NAME@ 组件外观一致。原生元素也可以支持与组件相同的外观、颜色和尺寸工具类。
